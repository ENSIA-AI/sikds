...
class DocumentsController
{
    ...
    public function index(Request $request): View
    {
        ...
        $dateFrom = $this->toTimestampFromIsoDate($filters['date_from']);
        $dateTo = $this->toTimestampFromIsoDate($filters['date_to']);
        $statusFilters = array_map('strval', $filters['status']);
        $tagFilters = array_map('strval', $filters['tags']);
        $audienceFilters = array_map('strval', $filters['audience']);

        $documents = array_values(array_filter($documents, function (array $doc) use (
            $q,
            $dateFrom,
            $dateTo,
            $statusFilters,
            ...
            if ($q !== '') {
                $haystack = mb_strtolower($doc['title'].' '.$doc['reference']);
                if (! str_contains($haystack, $q)) {
                    return false;
                }
            }

            $issueDate = $this->toTimestampFromFrenchDate($doc['issue_date']);

            if ($dateFrom !== null && ($issueDate === null || $issueDate < $dateFrom)) {
                return false;
            }

            if ($dateTo !== null && ($issueDate === null || $issueDate > $dateTo)) {
                return false;
            }

            if ($statusFilters !== [] && ! in_array((string) $doc['status'], $statusFilters, true)) {
                return false;
            }

            if ($tagFilters !== []) {
                $docTags = array_map(static fn (array $tag): string => (string) $tag['label'], $doc['tags']);
                if (count(array_intersect($docTags, $tagFilters)) === 0) {
                    return false;
                }
            }

            if ($audienceFilters !== [] && ! in_array((string) $doc['target_audience'], $audienceFilters, true)) {
                return false;
            }
    ...
    private function toTimestampFromFrenchDate(string $value): ?int
    {
    ...
    private function toTimestampFromIsoDate(string $value): ?int
    {
        if ($value === '') {
            return null;
        }
