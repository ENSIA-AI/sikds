@extends('layouts.app')
@section('page_title', __('Détail du Filigrane'))
@section('page_subtitle', __('Traçabilité du filigrane de téléchargement'))
@section('content')
    @php
    ...
    <x-back-link :href="route('watermark.index')" :label="__('Retour à la traçabilité')" class="mb-3" />
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <div class="flex items-center gap-2 mb-0.5">
                <h2 class="font-semibold text-lg" style="color:var(--sikds-ink)">{{ __('Détail du Filigrane') }}</h2>
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold" style="background:#dcfce7;color:#15803d;">&#10003; {{ __('Vérifié') }}</span>
            </div>
        ...
        <div class="lg:col-span-2 space-y-5 min-w-0">
            ...
            <div class="bg-white rounded-[14px] border p-6" style="border-color:rgba(0,0,0,.1);box-shadow:var(--sikds-shadow-panel);">
                <div class="flex items-center gap-2 mb-5">
                    ...
                    <h2 class="font-semibold text-base" style="color:var(--sikds-ink)">{{ __('Informations du Document') }}</h2>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="font-bold text-base" style="color:var(--sikds-ink)">{{ $log->document?->title ?? '-' }}</p>
                        <p class="text-xs mt-1" style="color:var(--sikds-muted)">{{ __('Référence') }}&nbsp;: {{ $log->document?->reference_number ?? '-' }}</p>
                        <p class="text-xs mt-0.5" style="color:var(--sikds-muted)">{{ __('Version') }}&nbsp;: {{ $log->document?->version_number ?? '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[14px] border p-6" style="border-color:rgba(0,0,0,.1);box-shadow:var(--sikds-shadow-panel);">
                <div class="flex items-center gap-2 mb-5">
                    ...
                    <h2 class="font-semibold text-base" style="color:var(--sikds-ink)">{{ __('Informations de l\'Utilisateur') }}</h2>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('Nom complet') }}</p>
                        <p class="font-semibold text-sm break-words" style="color:var(--sikds-ink)">{{ $log->user?->full_name ?? $log->user?->username ?? '-' }}</p>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('ID Utilisateur') }}</p>
                        <p class="text-sm font-mono break-all" style="color:var(--sikds-ink)">{{ $userId }}</p>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('Email') }}</p>
                        <p class="text-sm break-all" style="color:var(--sikds-ink)">{{ $log->user?->email ?? '-' }}</p>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('Institution') }}</p>
                        <div class="flex items-start gap-1.5">
                            <img src="/building-blue.svg" alt="" class="h-3.5 w-3.5 opacity-60 mt-0.5 shrink-0">
                            <p class="text-sm break-words" style="color:var(--sikds-ink)">{{ $log->user?->institution?->name ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[14px] border p-6" style="border-color:rgba(0,0,0,.1);box-shadow:var(--sikds-shadow-panel);">
                <div class="flex items-center gap-2 mb-5">
                    ...
                    <h2 class="font-semibold text-base" style="color:var(--sikds-ink)">{{ __('Informations du Téléchargement') }}</h2>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('ID Téléchargement') }}</p>
                        <p class="text-sm font-mono break-all" style="color:var(--sikds-ink)">{{ $downloadId }}</p>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('Statut') }}</p>
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold" style="background:#dcfce7;color:#15803d;">&#x25CF; {{ __('Complété') }}</span>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('Date & Heure') }}</p>
                        <div class="flex items-start gap-1.5">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs mb-0.5" style="color:var(--sikds-muted)">{{ __('Adresse IP') }}</p>
                        <div class="flex items-start gap-1.5">
                ...
                @if ($log->user_agent)
                    <div class="mt-3">
                        <p class="text-xs mb-1" style="color:var(--sikds-muted)">{{ __('User Agent') }}</p>
            ...
            <div class="bg-white rounded-[14px] border p-6" style="border-color:rgba(0,0,0,.1);box-shadow:var(--sikds-shadow-panel);">
                <div class="flex items-center gap-2 mb-5">
                    ...
                    <h2 class="font-semibold text-base" style="color:var(--sikds-ink)">{{ __('Événement d\'Audit Lié') }}</h2>
                </div>
                @if ($auditEntry)
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <p class="font-semibold text-sm" style="color:var(--sikds-ink)">{{ __('Action') }}&nbsp;: {{ $auditEntry->event_type }}</p>
                            <p class="text-xs mt-1" style="color:var(--sikds-muted)">{{ __('ID') }}&nbsp;: {{ $auditId }} | {{ $auditEntry->created_at?->format('Y-m-d H:i:s') }}</p>
                        </div>
                        <a href="{{ route('audits.index', [
                           ...
                           style="color:var(--sikds-primary)">
                            ...
                            {{ __('Voir dans les audits') }} &rarr;
                        </a>
                    </div>
                @else
                    <x-alert-item type="info" :message="__('Aucun événement d\'audit associé à ce téléchargement.')" timestamp="" />
                @endif
            </div>
        </div>
        ...
        <div class="lg:col-span-1 space-y-5 min-w-0">
            ...
            <div class="bg-white rounded-[14px] border p-5" style="border-color:rgba(0,0,0,.1);box-shadow:var(--sikds-shadow-panel);">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <h2 class="font-semibold text-sm" style="color:var(--sikds-ink)">{{ __('Filigrane Visible') }}</h2>
                </div>
                <div class="space-y-3 text-sm">
                    <div>
                        <p class="text-xs mb-1" style="color:var(--sikds-muted)">{{ __('En-tête') }}</p>
                        <div class="rounded-[10px] px-3 py-2 font-mono text-xs break-all" style="background:var(--sikds-primary);color:#ffffff">
                            {{ ($log->user?->full_name ?? $log->user?->username) }} | {{ $log->user?->institution?->name ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <p class="text-xs mb-1" style="color:var(--sikds-muted)">{{ __('Pied de page') }}</p>
                        <div class="rounded-[10px] px-3 py-2 font-mono text-xs break-all" style="background:#f6f6f6;color:var(--sikds-ink)">
                            {{ __('Téléchargé le') }} {{ $log->downloaded_at?->format('Y-m-d') }} {{ __('à') }} {{ $log->downloaded_at?->format('H:i') }} | UUID: {{ $shortUuid }}
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-2 text-xs pt-1">
                        <div>
                            <p style="color:var(--sikds-muted)">{{ __('Position') }}</p>
                            <p class="font-medium mt-0.5" style="color:var(--sikds-ink)">{{ __('Diagonal & Footer') }}</p>
                        </div>
                        <div>
                            <p style="color:var(--sikds-muted)">{{ __('Opacité') }}</p>
                            <p class="font-medium mt-0.5" style="color:var(--sikds-ink)">40%</p>
                        </div>
                        <div>
                            <p style="color:var(--sikds-muted)">{{ __('Couleur') }}</p>
                            <p class="font-medium mt-0.5 font-mono" style="color:var(--sikds-ink)">{{ __('Gray (#666666)') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[14px] border p-5" style="border-color:rgba(0,0,0,.1);box-shadow:var(--sikds-shadow-panel);">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <h2 class="font-semibold text-sm" style="color:var(--sikds-ink)">{{ __('Métadonnées PDF') }}</h2>
                </div>
                <div class="space-y-2">
                    @foreach ($meta as $key => $value)
                        <div class="flex flex-col gap-0.5 py-1 border-b last:border-0" style="border-color:rgba(0,0,0,.06);">
                            ...
                            <span class="text-xs font-mono break-all" style="color:var(--sikds-ink)">{{ $value ?? '-' }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
